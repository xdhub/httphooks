...
function validateHttpContextResponse(response) {
    if (typeof response !== 'object' && response !== null) {
        throw new Error('The provided httpContext response is invalid');
    }
}

function validateHttpContextResponseQueue(responses) {
    if (!(responses instanceof Array)) {
        throw new TypeError('The provided httpContext response queue is invalid');
    }
}
 ...
 * @member {HookContextResponse} [response] The response object. *Only provided for any responder or output-listener hook.*
...
var HookContext = function (httpContext, hook) {
    ...
    validateHttpContextResponseQueue(httpContext.responseQueue);
    this.request = HookContextRequest.createFromHttpRequest(httpContext.request);
    this.responseQueue = clone(httpContext.responseQueue);
    ...
    if (isResponderHook) {
        ...
        if (hook.type === 'output-listener') {
            validateHttpContextResponse(httpContext.response);
            this.response = clone(httpContext.response);
        }
    }
};
