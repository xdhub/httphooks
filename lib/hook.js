...
function isValidUri(uri) {
    var normalizedUri = url.parse(uri);
    return isValidUrlPath(normalizedUri) && isSupportedUrlProtocol(normalizedUri);
}

function isValidHookCallback(callback) {
    var valid = typeof callback === 'object';
    if (valid) {
        valid = typeof callback.uri === 'string' && isValidUri(callback.uri);
        if (valid) {
            if (callback.uri.indexOf('file:') === 0) {
