...
function createFunctionFromCallbackInfo(callback) {
    // TODO: Loading of functions should be delayed until an incoming request
    //       requires it. For not we will load upon normalization
    var module = null;
    var func = null;
    var uri = url.parse(callback.uri);

    // TODO: Add support for other protocols such as HTTP and HTTPS in order
    //       to be able to reference remote functions that will be invoked.
    //       For remote hooks, I can think of two hook models:
    //       1) Download remote code in order to invoke it locally
    //       2) Forward the request to the remote system
    //       So far I am inclined on 2), since 1) is basically management of remote
    //       files and could be considered out of scope.
    var localPath = null;
    if (uri.protocol === 'file:') {
        localPath = uri.path;
        if (localPath.charAt(0) === '/') {
            localPath = localPath.substr(1, localPath.length - 1);
        }
    }
...
function isValidRequest(httpContext) {
    var valid = Hook.isValidRequestMethod(httpContext.request.method);
    if (!valid) {
...
function createHookContext(hookExecutionContext) {
...
function executeResponderHooks(hooks, httpContext, stage, chainCallback) {
    async.eachSeries(
        ...
        function (error) {
            ...
            // returned an unsuccesful status code, or we are at the end of the
...
function processPostResponderHookContinueResponse(hookExecutionContext, hookContext) {
    ...
    var response = getResponseFromHookResponseContent(hookExecutionContext, hookContext);
    if (response) {
        hookExecutionContext.httpContext.responseQueue.length = 0;
        var shortenedResponse = HookContextResponse.createResponse(
            response.statusCode,
            response.headers,
            response.content);
        hookExecutionContext.httpContext.responseQueue.push(shortenedResponse);
    }
}
...
HttpHooks.prototype.onNoMatch = function (callback) {
    var valid = Hook.isValidCallback(callback);
    if (!valid) {
...
function processingStage(hooks, httpContext, chainCallback) {
    var type = 'responder';
    ...
    if (hooks.length > 0) {
        ...
        async.series([
            ...
            function (cb) {
                ...
                processingStage(hooks, httpContext, cb);
            },
            function (cb) {
        ...
        function (error, result) {
            ...
            // Replace the context response with the one sent
