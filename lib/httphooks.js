...
function dispatchWebSocketRequest(socketContext) {
    socketContext.on('request', function (request, responseCallback) {
        if (isValidWebSocketRequest(request)) {
            var httpContext = socketIoHelper.createHttpContextFromSocketIo(socketContext, request, responseCallback);
            processHttpRequest.call(this, httpContext);
        } else {
