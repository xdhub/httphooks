...
var sockJsHelper = require('./sockjs/index.js');
 ...
 * @member {string} framework The socket framework to use: `'socket.io' | 'sockjs'`.
 */
...
function executeHook(hookExecutionContext, executionCompleteCallback) {
    ...
    try {
        ...
        executionCompleteCallback(hookContext, errorAsString, true);
    }
}
...
function dispatchSocketIoRequest(context) {
    var that = this;
    context.socket.on('request', function (request, responseCallback) {
        if (isValidSocketRequest(request)) {
            ...
            debug('Skipping socket.io request dispatching since its invalid');
            context.disconnect();
        }
    });
...
function dispatchSockJsRequest(context) {
    var that = this;
    context.socket.on('data', function (requestAsText) {
        debug('Received data request from ' + context.socket.remoteAddress + ':' + context.socket.remotePort);
        var request = JSON.parse(requestAsText);
        if (isValidSocketRequest(request)) {
            var httpContext = sockJsHelper.createHttpContextFromSocketJs(context.socket, request);
            httpContext.responseQueue = [];
            processHttpRequest.call(that, httpContext);
        } else {
            debug(
                'Skipping SockJS request from ' + context.socket.remoteAddress + ':' + context.socket.remotePort +
                'dispatching since the request its invalid');
            context.socket.close();
        }
    });
    context.socket.on('close', function () {
        debug('Connection closed for ' + context.socket.remoteAddress + ':' + context.socket.remotePort);
    });
}
...
function dispatchRequest(context) {
    ...
    } else if (context.socket && context.framework === 'socket.io') {
        debug('Incoming socket.io request');
        dispatchSocketIoRequest.call(this, context);
    } else if (context.socket && context.framework === 'sockjs') {
        debug('Incoming sockjs request');
        dispatchSockJsRequest.call(this, context);
    } else {
