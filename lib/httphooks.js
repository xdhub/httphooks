...
function executeHookWithResponseReplacement(hook, httpContext, executionCompleteCallback)

function executeListenerHooks(hooks, httpContext, response) {
    async.each(
...
function outputStage(hooks, httpContext, response, chainCallback) {
...
function processRequest(httpContext) {
    ...
    if (hooks.length > 0) {
        ...
        function (error, result) {
            ...
            outputStage(hooks, httpContext, response, function () {});
        });
    } else {
        debug(
