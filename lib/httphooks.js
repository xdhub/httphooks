...
/**
 * Create a WebSocketContext object
 * @class WebSocketContext
 * @member {object} socket The web socket object.
 */
...
function getHookTypes(hookTypesList) {
    ...
    if (hookTypesList) {
        ...
        hookTypes = hookTypes.map(function (element) {
            ...
            return Hook.isValidHookType(element);
        });
    }
...
function getHooks(httpContext) {
    ...
    var hookTypesList = httpContext.request.headers['x-httphooks-hook-type'];
...
function isValidWebSocketRequest(request) {
    var valid = Hook.isValidRequestMethod(request.method);
    if (valid) {
...
function dispatchWebSocketRequest(context) {
    var that = this;
    context.webSocket.on('request', function (request, responseCallback) {
        if (isValidWebSocketRequest(request)) {
            var httpContext = socketIoHelper.createHttpContextFromSocketIo(context.webSocket, request, responseCallback);
            httpContext.responseQueue = [];
            processHttpRequest.call(that, httpContext);
        } else {
...
function dispatchRequest(context) {
    if (context.request && context.response) {
        ...
        dispatchWebSocketRequest.call(this, context);
    } else {
 ...
 * @method
 * @param {HttpContext | WebSocketContext} context The context that contains the request.
 */
